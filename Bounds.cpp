...
double UpBounds::bounds1(ARG_3, double sigm_){
		...
		auto return_value1 = std::function<double(void)>([&]() {edit_fild(5); return double(1); });
		auto return_value2 = std::function<double(void)>([&]() {edit_fild(sigm_ > 0 ? 2 : 1); return Z(e, t, s, sigm_ * 8. * s + 4. * t); });
		if (t > -0.5 && t < 0.)
...
double UpBounds::bounds2(ARG_3) {
	auto return_value1 = std::function<double(void)>([&]() {edit_fild(5); return double(1); });
	auto return_value2 = std::function<double(void)>([&]() {edit_fild(3); return Z(e, t, s, 4. * sqr_(s) / t); });
	if ((0. < t) && (t < 0.5))
		std::swap(return_value1, return_value2);
...
double DownBounds::bounds1(ARG_3, double sigm){
		auto return_value1 = std::function<double(void)>([=]() {edit_fild(4); return double(0); });
		auto return_value2 = std::function<double(void)>([=]() {edit_fild(sigm > 0 ? 2 : 1); return Z(e, t, s, sigm * 8. * s + 4. * t); });
		if (std::abs(t) > 0.5)
			std::swap(return_value1, return_value2);
